...
- **Role:** Primary sharp O/U + milestone (`N+`) ladders; main input to `line_adjustment.py`.
...
- **De-vig (milestone):** `devig_milestone_fair_over` in `line_adjustment.py` (ladder-normalize else hold-shrink; `MILESTONE_MIN_FAIR_OVER` gate).
...
- **EV resolution:** Per book `resolve_book_sharp_quote` (O/U preferred, else milestone). Multi-book consensus when 2+ exact O/U (`SHARP_BOOK_WEIGHTS_*`). One EV row per Betr side. `filter_min_ev` / `--plus-ev-only` / `--min-ev`.
...
└── backend/
    ├── config/                         # headers, market_maps, team_abbrev, settings, *_{subcategories,markets,competitions,queries}, pipeline_sources
    ...
    ├── core/
    │   ├── models.py, engine.py, line_adjustment.py, flat_line.py
    ...
    └── tests/                          # fixtures, integration, unit; 546 tests
```
