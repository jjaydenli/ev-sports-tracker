...
**Last verified:** 2026-06-19 (event_start hour-floor validation; cross-book game + live snapshot matching on sharp ladders)
...
* **Detail:** [docs/betting_odds/mlb.md](docs/betting_odds/mlb.md) — pregame 13-market O/U slate (`DK_MLB_STAT_CATEGORIES`, incl. `doubles`) plus live batter O/U (`DK_MLB_LIVE_STAT_CATEGORIES` — all 8 batter live IDs set, e.g. `walks` `9536` live vs `17411` pregame; live IDs differ from pregame on many tabs). FanDuel MLB: same 13-market pregame O/U list (pitcher + batter tabs) plus milestone `N+` boards on those tabs (hits, total bases, runs, RBI, H+R+RBI); `./ev --mlb --books fd` or full `--mlb` Betr ↔ (DK + FD) consensus and milestone gap-fill. EV output rows include `is_live` (ranked table **Live** column). **Cross-book matching:** sharp O/U and milestone ladders key on `player|market|[game]|[live]` (`build_player_market_key` in `line_adjustment.py`) so a live Betr prop for tonight's `CIN@NYY` does not pick up tomorrow's pregame DK/FD lines for the same team pairing; after sharp resolution, `find_ev_opportunities` drops rows where Betr `event_start` hour-floor (`iso[:13]`) disagrees with the contributing sharp book's start at the resolved line (doubleheaders separated by >2h gap). No CLI flag — DK and Betr live discovery are standing behavior on `./ev --leagues mlb` (Betr: app-parity GraphQL headers; DK: `DK_MLB_LIVE_STAT_CATEGORIES`).
...
└── backend/
    ...
    ├── core/
    │   ├── models.py
    │   ├── engine.py           # find_ev_opportunities; event_start hour-floor filter; filter_min_ev; milestone row fields
    │   ├── line_adjustment.py  # build_player_market_key (game + live scope); O/U resolve; milestone de-vig; multi-book consensus
    │   ├── flat_line.py
    │   ├── ev_pipeline.py
